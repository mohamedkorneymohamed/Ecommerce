...
let {getLoggedUserCart , deleteCartDetails ,setCartNumber , updateProductCount} = useContext(cartContext)
...
</div></div>: <>{/*----------------------------------- Display All Selected Product In Card---------------------------------- */}
          <Helmet>
            <title>Cart Details</title>
        </Helmet>
  {cartDetails?  <div className=" vh-100 bg-main-light p-4">
<h3>Shop Cart :</h3>
<h6 className='text-main'> Total Cart Price :{cartDetails.totalCartPrice}EGP</h6>
{cartDetails.products.map((product)=><div key={product?.product?._id} className="row py-2 my-2 align-items-center">
...
<img src={product?.product?.imageCover} className='w-100' alt="" />
  ...
  <div className="col-md-11 d-flex justify-content-between ">
    ...
    <button onClick={()=>deleteProduct(product?.product?._id)} className='btn text-danger'><i className='fa-regular fa-trash-can'></i> remove</button>
    </div>
<div>
